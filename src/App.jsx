...
function Recipes() {
  ...
  const handleDelete = async (id) => {
    ...
    setDeleting(id);
    try { await fetch(`https://backend-repo-psi.vercel.app/api/recipes/${id}`, { method:'DELETE', headers }); fetchAll(); }
  ...
  return (
    <div>
      {showUpgrade && <UpgradeModal reason={`Free accounts can only create up to ${limits.limits.recipes} products. Upgrade for unlimited recipes.`} onClose={()=>setShowUpgrade(false)}/>}
      ...
      <div style={{...styles.row,justifyContent:'space-between',marginBottom:8}}>
        <div>
          ...
          <div style={styles.pageSub}>Manage products and menu items</div>
        </div>
        <button style={styles.btnPrimary} onClick={openAdd}>+ Add Product</button>
      </div>

      <div style={styles.grid2}>
        <div style={styles.statCard}><div style={styles.statLabel}>Total Products</div><div style={styles.statValue}>{recipes.length}</div><div style={styles.statSub}>Active products {limits?.isLimited ? `(${limits.usage.recipes}/${limits.limits.recipes} free limit)` : ''}</div></div>
      ...
      {loading ? (
        ...
        <div style={{textAlign:'center',padding:'40px 0',color:COLORS.gray400}}>
          {recipes.length === 0 ? 'No recipes yet. Click "+ Add Recipe" to get started.' : 'No products match your search.'}
        </div>
      ) : (
          ...
          {filtered.map(r=>(
            <div key={r.id} style={styles.card}>
              ...
              <div style={{...styles.row,justifyContent:'flex-end',marginTop:14,gap:8}}>
                <button style={styles.btnOutline} onClick={()=>openEdit(r)}>Edit Product</button>
...
function Purchases() {
  ...
  const handleDelete = async (id) => {
    ...
    setDeleting(id);
    try {
      await fetch(`https://backend-repo-psi.vercel.app/api/purchases/${id}`, { method: 'DELETE', headers });
      fetchAll();
    } finally { setDeleting(null); }
  };
          ...
          <div style={styles.card}>
            ...
            {isPremium ? (
              data.productMargins.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '20px 0', color: COLORS.gray400, fontSize: 13 }}>No products yet.</div>
              ) : (
                <div style={styles.tableWrap}>
                  <table style={{ ...styles.table, marginTop: 8 }}>
                    <thead>
...
function AdminUserActions({ user, onRefresh }) {
  ...
  const upgrade = async (billingCycle) => {
    setLoading(true);
    await fetch(`https://backend-repo-psi.vercel.app/api/premium/admin/upgrade/${user.id}`,{method:'PUT',headers,body:JSON.stringify({billingCycle})});
    setLoading(false); onRefresh();
  };
  const downgrade = async () => {
    if(!window.confirm('Downgrade this user to free?')) return;
    setLoading(true);
    await fetch(`https://backend-repo-psi.vercel.app/api/premium/admin/downgrade/${user.id}`,{method:'PUT',headers});
    setLoading(false); onRefresh();
  };
...
function AdminSubs() {
  ...
  const handleApprove = async (reqId, userId, billingCycle) => {
    ...
    try {
      // Upgrade the user
      await fetch(`https://backend-repo-psi.vercel.app/api/premium/admin/upgrade/${userId}`, {
        method:'PUT', headers,
        body: JSON.stringify({ billingCycle }),
      });
      // Mark request as approved
      await fetch(`https://backend-repo-psi.vercel.app/api/admin/upgrade-requests/${reqId}/approve`, {
        method:'PUT', headers,
      });
      // Refresh
      const [sRes, rRes] = await Promise.all([
